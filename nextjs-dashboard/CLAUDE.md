...
| `/familienausfluege` | SSG | Ausflugsziele mit Kindern in 4 Gruppen (Tiere hautnah, Erlebniswelten & Freizeitparks, Baden mit flachem Einstieg, Kurze Wege/große Natur) mit „Familien-Tipp" je Ziel + genereller „Familien-Check"-Box (Kinderwagen/Trage, Zeiten vorab prüfen, Wechselkleidung). Jeder Tipp stützt sich auf den verlinkten Artikel. Daten in `app/lib/themen-picks.ts` (`FAMILIEN_GROUPS`), im Rückwärts-Index mit rank 16 |
...
| `/unterkuenfte/basislager` | SSG | Affiliate-Landingpage „Basislager“: je Bundesland die Unterkunft-Standorte aus `unterkuenfte.ts` mit den meisten Artikel-Zielen im Umkreis von 25 km Luftlinie (`app/lib/basislager.ts` → `basislagerByRegion()`, nutzt `coordsOf()`/`distKm()` aus `wochenendtrip.ts`; `Unterkunft`-Artikel ausgenommen, mind. 3 Ziele). Karte je Standort mit Kategorie-Zählern, den 6 nächsten Zielen und `cloak()`-Buchungslink. Rein berechnet – wächst automatisch mit neuen Artikeln/Unterkünften. `ItemList`+`BreadcrumbList`-JSON-LD |
