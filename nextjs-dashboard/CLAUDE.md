...
- **"Mit Öffis erreichbar"-Badge**: same file → `oeffiAnreise(slug)` looks up the post in `BAHNHOF_GROUPS` (from `themen-picks.ts`) and returns its curated `anreise` text. [app/blog/[slug]/page.tsx](app/blog/[slug]/page.tsx) shows a header badge + a sidebar card with the concrete transit instructions for the ~29 posts already curated on `/bahnhofsausfluege`. No new `Post` field — reuses the existing curated list instead of tagging all ~165 posts individually. A second, boolean-only helper `isOeffiErreichbar(slug)` (same file, backed by a `Set` over `BAHNHOF_GROUPS`) drives a compact 🚋-badge on the overview cards too — [app/ui/blog-search.tsx](app/ui/blog-search.tsx) (magazine grid) and [app/regionen/[bundesland]/page.tsx](app/regionen/[bundesland]/page.tsx) (region-page post cards) — without pulling in the full anreise text on every card.
