...
- `/unterkuenfte/romantisch` — kommerzielle Intent-Seite (Affiliate); `/unterkuenfte/am-see` ist seit 2026-09-16 live. Für „romantisch" fehlt noch ein geeignetes Kriterium in den Daten (kein `typ`/Tag dafür) — erst umsetzen, wenn eine sinnvolle, nicht erfundene Abgrenzung existiert (z. B. Glamping + kleine Ferienwohnungen ohne Camping).
...
- „Mit Öffis erreichbar"-Badge existiert seit 2026-08-23 auf der Artikelseite und seit
  2026-09-03 auch auf Magazin-Grid + Regionsseiten-Karten (`isOeffiErreichbar()`). Am
  2026-09-11 von ~20 auf 29 kuratierte Artikel in `BAHNHOF_GROUPS` erweitert. **Offen:**
  bei Bedarf weitere klare Bahn-Ziele nachziehen (z. B. Wörgl, Villach, Bad Ischl –
  sofern dafür eigene Artikel entstehen).
...
- **Neue Artikel** (Slug vorher prüfen!) — dünn besetzte Regionen zuerst.
  Stand 2026-09-17 je Region: Kärnten 44, Burgenland 26, Steiermark 26, Salzburg 19, Tirol 19,
  Wien 12, Oberösterreich 12, Niederösterreich 12, Vorarlberg 11 (jetzt alleine dünnste Region).
  OÖ (Innviertel/Schärding, Ried/Antiesental, Bad Ischl), NÖ (Pielachtal/Dirndlweg, Carnuntum),
  Vorarlberg (Hohenems) und Wien (Hundertwasserhaus, Naschmarkt) aus dem Backlog sind erledigt.
  Wolfgangsee-Ostufer/St. Wolfgang bewusst **nicht** genommen — überschneidet sich zu
  stark mit dem bestehenden Artikel `wolfgangsee-st-gilgen` (Salzburg-Seite desselben Sees,
  inkl. Schafbergbahn). Kahlenberg/Leopoldsberg für Wien ebenfalls bewusst übersprungen — bereits
  vollständig als Tour in `wienerwald-wandern` (inkl. `routeVariants`) abgedeckt.
...
- 2026-09-17 — [Content] 2 neue Artikel für die bis dahin dünnsten Regionen (Wien, Oberösterreich, je 11): `naschmarkt-wien` (Wien, Ausflug) – Wiens größter Markt zwischen Karlsplatz und Kettenbrückengasse, mit Geschichte (Name seit 1820 belegt, offiziell seit 1905, heutige Form durch Otto Wagners Stadtbahnbau um 1900), dem seit 1977 angrenzenden Flohmarkt und Otto Wagners Jugendstilhäusern (Majolikahaus); verlinkt mit `museumsquartier-wien` (Rückverweis dort ergänzt) und neu in `BAHNHOF_GROUPS` (`themen-picks.ts`) aufgenommen, da Karlsplatz/Kettenbrückengasse direkt am Markt liegen. `bad-ischl-kaiservilla` (Oberösterreich, Ausflug) – die „heimliche Hauptstadt der Donaumonarchie": Kaiservilla als Sommerresidenz von Franz Joseph und Sisi (inkl. des dort unterzeichneten Manifests „An meine Völker" von 1914), Kurstadt-Geschichte und die Rolle als Europäische Kulturhauptstadt 2024 (gemeinsam mit 22 Salzkammergut-Gemeinden); verlinkt mit `hallstatt-salzkammergut` (dort „Bad Ischl" jetzt als Link statt reinem Text), `gosausee-dachstein-spiegelung` und `dachstein-krippenstein`. Beide Artikel vor dem Schreiben per Websuche faktengeprüft (Kaiservilla-Geschichte, Naschmarkt/Otto-Wagner-Fakten, Kulturhauptstadt 2024), bewusst ohne exakte Preise/Öffnungszeiten. SEO-Keywords „Naschmarkt" (Wien) und „Bad Ischl" (OÖ) in `app/lib/seo.ts` ergänzt. Damit ist Vorarlberg (11) jetzt alleine die dünnste Region, Wien, Oberösterreich und Niederösterreich liegen gleichauf (je 12).
- 2026-09-16 — [Feature] Neue Affiliate-Landingpage `/unterkuenfte/am-see`: bündelt Hotels/Ferienwohnungen/Camping mit direkter Lage an einem Badesee, nach Bundesland gruppiert. Neues `unterkuenfteAmSee()` in `app/lib/unterkuenfte.ts` filtert die bestehenden 33 Unterkünfte über eine Whitelist echter Seenamen (`BADESEE_NAMEN`) — grenzt Flüsse (Donau, Inn, Mur, Salzach, Enns), Täler/Berge (Ötztal, Montafon, Dachstein, Ötscher, Großglockner, Nockberge, Rax-Schneeberg) und die Therme Lutzmannsburg im bestehenden `see`-Feld aus, ohne neue Fakten zu erfinden. Ergibt 20 Unterkünfte in 7 Bundesländern (nur Steiermark hat aktuell keine Seen-Unterkunft gelistet). Neues `lakeSlugFor()` verlinkt optional zum passenden `/blog`-Artikel aus `LAKES` (`app/lib/seen.ts`), inkl. Sonderfall „Bodensee" → „Bodensee (Bregenz)". In `sitemap.ts`, Footer, `/ausflugsplaner`-Hub sowie von `/seen-vergleich` und den Regions-Unterkünfte-Boxen aus verlinkt; zusätzlich neue Quelle (rank 15) im Rückwärts-Index `themenseiten.ts`, sodass z. B. der Wörthersee-Artikel jetzt auch auf die Buchungsseite zurückverweist. Bewusst nur Richtwert-„ab"-Preise (bereits vorhandene Daten), keine exakten Verfügbarkeiten behauptet.
- 2026-09-15 — [Content] 2 neue Artikel für die bis dahin dünnsten Regionen: `hohenems-rosenstadt-schloss` (Vorarlberg, Ausflug) – die „Rosenstadt" im Rheintal mit Renaissanceschloss der Grafen von Hohenems (Schauplatz der Schubertiade) und dem gut erhaltenen jüdischen Viertel samt Jüdischem Museum, eine der bedeutendsten jüdischen Landgemeinden im heutigen Österreich; verlinkt mit `feldkirch-altstadt-schattenburg` (Rückverweis dort ergänzt). `carnuntum-archaeologiepark` (Niederösterreich, Ausflug) – Archäologiepark der römischen Zivilstadt Carnuntum bei Petronell, das freistehende Heidentor und das Museum Carnuntinum in Bad Deutsch-Altenburg, östlich von Wien; als donauabwärtiger Kontrast zu `wachau-duernstein` verlinkt (Rückverweis dort ergänzt). SEO-Keywords „Hohenems" und „Carnuntum" in `app/lib/seo.ts` ergänzt. Bewusst ohne exakte Öffnungszeiten/Preise, stattdessen „vorab prüfen"-Hinweise, da beide Ziele saisonal/wetterabhängig geöffnete Freilichtbereiche haben. Damit sind Oberösterreich und Vorarlberg (je 11) jetzt gemeinsam die dünnsten Regionen.
- 2026-09-14 — [Feature] Neue Landingpage `/zwei-ausfluege-an-einem-tag`: kombiniert je aktiver Region bis zu 4 Paare aus zwei nah beieinander liegenden, halbtagstauglichen Zielen (Vormittag + Nachmittag) — neues `app/lib/tagescombo.ts` (`combosByRegion()`), fast 1:1 nach dem Muster von `app/lib/wandern-baden.ts` (Haversine über `startCoords`/`trails[0]`, ≤15 km, greedy Nearest-Neighbor-Pairing, max. 4 je Region). Unterkünfte sind ausgeschlossen, anspruchsvolle Wanderungen (`difficulty` mittel/schwer) ebenfalls, da die einen Tag allein füllen. Vor dem Commit per Python-Simulation gegen alle 177 Artikel gegengeprüft: 28 sinnvolle Paare in allen 9 Bundesländern (z. B. Salzburg-Altstadt + Schloss Hellbrunn, Rax-Seilbahn + Semmeringbahn, Stephansdom + Hofburg). In `sitemap.ts`, Footer und `/ausflugsplaner`-Hub verlinkt, Querverlinkung zu/von `/wandern-baden` und `/ausfluege-nach-dauer`, sowie neue Quelle (rank 14) im Rückwärts-Index `themenseiten.ts`. Bewusst nur Entfernung in km statt einer Fahrzeit-Behauptung, passend zur „keine erfundenen Fakten"-Regel.
- 2026-09-13 — [Content] 2 neue Artikel für die bis dahin dünnsten Regionen: `hundertwasserhaus-kunsthauswien` (Wien, Ausflug) – Hundertwasserhaus (bewohntes Gemeindebau-Wohnhaus, nur von außen zu besichtigen, bewusst ehrlich als solches benannt statt es als begehbare Attraktion darzustellen) und das direkt danebenliegende KunstHausWien-Museum, verlinkt mit `schloss-belvedere-wien` und dem thematisch verwandten `rogner-bad-blumau-therme` (ebenfalls Hundertwasser, andere Region); zusätzlich in die „Städte bei Regen"-Gruppe auf `/regentaugliche-ausfluege` aufgenommen. Kahlenberg/Leopoldsberg als naheliegende Wien-Idee bewusst verworfen, da dort bereits durch `wienerwald-wandern` (inkl. `routeVariants` für genau diese Runde) abgedeckt – Duplicate-Content-Check vor dem Schreiben hat das aufgedeckt. `ried-im-innkreis-antiesental` (Oberösterreich, Ausflug) – Bezirkshauptstadt des Innviertels abseits von Schärding, dazu das ruhige, flache Antiesental mit Streuobstwiesen; Rückverweise zwischen diesem und `schaerding-altstadt-innviertel` in beide Richtungen ergänzt. SEO-Keywords „Ried im Innkreis" (OÖ) und „Hundertwasserhaus" (Wien) in `app/lib/seo.ts` ergänzt. Damit ist Vorarlberg (10) jetzt alleine die dünnste Region.
- 2026-09-11 — [Feature] „Mit Öffis erreichbar"-Abdeckung erweitert: `BAHNHOF_GROUPS` in `app/lib/themen-picks.ts` von 20 auf 29 kuratierte Artikel ausgebaut. Neu in „Ziel direkt an der Bahn": `murau-murtalbahn` (Murtalbahn-Nostalgiezug), `semmeringbahn-weltkulturerbe` (die Bahnstrecke selbst ist Thema des Artikels), `mariazell-wallfahrt-ausflug` (Mariazellerbahn endet im Ort), `bad-gastein-wasserfall`, `st-anton-am-arlberg`, `feldkirch-altstadt-schattenburg`, `eisenstadt-schloss-esterhazy`; neu in „Stadt & U-Bahn": `wien-stephansdom-altstadt` (U1/U3 Stephansplatz) und `klagenfurt-stadtfuehrung`. Da Badge (`isOeffiErreichbar`), Sidebar-Karte (`oeffiAnreise`) und die `/bahnhofsausfluege`-Seite selbst rein datengetrieben aus dieser Liste lesen, ziehen alle drei Stellen automatisch mit – kein Code in den Views geändert. Alle Anreise-Hinweise bewusst ohne exakte Fahrzeiten/Taktangaben formuliert (nur Gehminuten-Schätzungen bzw. "direkt am/im Ort"), passend zur „keine erfundenen Fakten"-Regel. Slugs gegen `posts.ts` geprüft.
- 2026-09-10 — [Content] 2 neue Artikel für die bis dahin dünnste Region (Oberösterreich, 9) und Niederösterreich: `schaerding-altstadt-innviertel` (Oberösterreich, Ausflug) – barocke Silberzeile am Inn im bislang unvertretenen Innviertel, Sternschanzen und die Nähe zu Passau, verlinkt mit `linz-ausflug` (Rückverweis dort ergänzt); `pielachtal-dirndlweg-frankenfels` (Niederösterreich, Wandern) – das „Dirndltal" um Frankenfels und Rabenstein an der Pielach im Mostviertel, benannt nach der Dirndlkirsche (Kornelkirsche), als ruhigere Ausgangsbasis Richtung Ötscher, verlinkt mit `oetscher-tormaeuer-wandern` (Rückverweis dort ergänzt). SEO-Keywords „Innviertel"/„Schärding" bzw. „Pielachtal"/„Mostviertel" in `app/lib/seo.ts` ergänzt. Damit ist Oberösterreich mit Vorarlberg und Wien gleichauf (je 10 Artikel).
- 2026-09-09 — [Feature] „Weite Anfahrt"-Übernachtungshinweis: neues `nearestCityKm()` in `app/lib/wochenendtrip.ts` berechnet die Luftlinie eines Artikels zu den 6 `TRIP_CITIES`. Liegt der Startpunkt weiter als 100 km von **allen** sechs Städten (aktuell nur die 9 Vorarlberg-Artikel betroffen, da jede andere Region näher an mindestens einer Stadt liegt) und gibt es eine Unterkunft der gleichen Region innerhalb von 60 km (Sicherheitsabstand gegen geografisch gespaltene Regionen wie Tirol/Osttirol – sonst hätte z. B. `lienzer-dolomiten-osttirol` eine 100+ km entfernte Innsbruck-Unterkunft vorgeschlagen bekommen), zeigt die Artikelseite eine Sidebar-Karte mit den 1–2 nächstgelegenen Unterkünften (Affiliate-Link). Bewusst keine konkrete Fahrzeit-Behauptung, da die Luftlinien-Näherung bei Gebirgsstraßen abweichen kann. Per Python-Simulation gegen alle 173 Artikel gegengeprüft, dass die Auswahl (9 Vorarlberg-Artikel, je 1–2 sinnvolle Unterkünfte von 0–35 km) stimmt. Wächst automatisch mit, sobald neue Artikel/Unterkünfte weit abseits der 6 Städte dazukommen (z. B. Osttirol, falls dort mal eine Unterkunft gelistet wird).
- 2026-09-08 — [Content] 2 neue Artikel für die bis dahin dünnsten Regionen: `feldkirch-altstadt-schattenburg` (Vorarlberg, Ausflug) – älteste Stadt Vorarlbergs im Walgau, Lauben-Altstadt, Schattenburg und der Wildpark am Ardetzenberg, verlinkt mit `bregenzerwald-wandern`, außerdem einen Rückverweis von `montafon-tal-wandern` ergänzt (Anfahrt über Bludenz); `nationalpark-thayatal-hardegg` (Niederösterreich, Wandern) – Flussschlucht der Thaya an der tschechischen Grenze bei Hardegg (Österreichs kleinster Stadt), grenzüberschreitend mit dem Nationalpark Podyjí, verlinkt mit `stausee-ottenstein-waldviertel` (dort ebenfalls Rückverweis ergänzt). Wolfgangsee-Ostufer aus dem Backlog bewusst übersprungen (Duplicate-Content-Risiko mit dem bestehenden `wolfgangsee-st-gilgen`). Damit ist Oberösterreich (9) jetzt alleine die dünnste Region, Niederösterreich und Vorarlberg liegen mit Wien gleichauf (je 10).
